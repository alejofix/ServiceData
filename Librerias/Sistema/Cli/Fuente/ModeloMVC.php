<?php
	...
	class ModeloMVC extends Command {
		...
		protected function configure() {
			$this->setName('fuente:mvc:modelo');
			$this->setDescription('Crear el Modelo solicitado en el MVC en entorno de Desarrollo');
			$this->addArgument('app', InputArgument::REQUIRED, 'Nombre de la aplicación');
			$this->addArgument('controlador', InputArgument::REQUIRED, 'Nombre del Modelo que se asignara');
		...
		private function escrituraControlador(InputInterface $input, OutputInterface $output) {
			$this->controladorDirectorio = implode(DIRECTORY_SEPARATOR, array(ROOT_APPS, ConfigAcceso::leer($this->parametro['app'], 'fuente', 'directorio'), 'Desarrollo', 'Fuente', 'Sistema', 'MVC', 'Modelos'));
			
			if(is_writable($this->controladorDirectorio) == true):
				$this->existenciaControlador($input, $output);
			else:
				throw new \RuntimeException(sprintf('El directorio de Modelos de la aplicación: [ %s ] no tiene permisos de lectura y escritura', $this->parametro['app']));
			endif;
		}
		...
		private function existenciaControlador(InputInterface $input, OutputInterface $output) {
			...
			if(file_exists($archivo) == false):
				...
				throw new \RuntimeException('El archivo de Modelo ya existe dentro del MVC');
			endif;
		}
		...
		private function escribirArchivo(InputInterface $input, OutputInterface $output, $archivo) {
			...
			$formato = (is_array($this->metodos) == true) ? $this->clasePlantilla($clase, implode("\n\n", $this->metodos)) : $this->clasePlantilla($clase, '');
			
			$fopen = fopen($archivo, 'w');
			fwrite($fopen, $formato);
			fclose($fopen);
			
			$output->writeln("\n\n<info>Se ha creado el modelo solicitado en el MVC</info>\n\n");
		}
...
EOT;
		 ...
		 * creacion del archivo del modelo
		 * 
		 * @param string $clase
