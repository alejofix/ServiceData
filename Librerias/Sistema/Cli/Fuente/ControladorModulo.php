<?php
	...
	class ControladorModulo extends Command {
		...
		protected function configure() {
			$this->setName('fuente:modulo:controlador');
			$this->setDescription('Crear el controlador solicitado en Modulo en entorno de Desarrollo');
		...
		private function escrituraControlador(InputInterface $input, OutputInterface $output) {
			$this->controladorDirectorio = implode(DIRECTORY_SEPARATOR, array(ROOT_APPS, ConfigAcceso::leer($this->parametro['app'], 'fuente', 'directorio'), 'Desarrollo', 'Fuente', 'Sistema', 'Modulos', $this->parametro['modulo'], 'Controladores'));
			
			if(is_writable($this->controladorDirectorio) == true):
				$this->existenciaControlador($input, $output);
			else:
				throw new \RuntimeException(sprintf('El directorio de Controladores del modulo: [ %s ] de la aplicación: [ %s ] no tiene permisos de lectura y escritura', $this->parametro['modulo'], $this->parametro['app']));
			endif;
		}
		...
		private function existenciaControlador(InputInterface $input, OutputInterface $output) {
			...
			if(file_exists($archivo) == false):
				...
				throw new \RuntimeException(sprintf('El archivo controlador ya existe dentro del Modulo [%s]', $this->parametro['modulo']));
			endif;
		}
		...
		private function escribirArchivo(InputInterface $input, OutputInterface $output, $archivo) {
			...
			$formato = $this->clasePlantilla($modulo, $clase, implode("\n\n", $this->metodos));
			
			$fopen = fopen($archivo, 'w');
			fwrite($fopen, $formato);
			fclose($fopen);
			
			$output->writeln("\n\n<info>Se ha creado el controlador solicitado en el Modulo</info>\n\n");
		}
		...
		/**
		 * ControladorModulo::constructorPlantilla()
		 * 
...
<?php
	 ...
	 */
	namespace Controlador\Modulo\\{$modulo};
